    ...
    <section className="my-container py-10">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2 items-center">
          <Link href="/categories" className="text-sm text-blue-600 hover:text-blue-800 font-medium mr-2">View All</Link>
            ...
            {loopItems.map((cat, idx) => (
              ...
              >
                <div className="relative aspect-[4/3] bg-white">
                  <Image src={cat.image} alt={cat.name} fill className="object-contain p-3 group-hover:scale-105 transition-transform" />
                </div>
                <p className="mt-2 text-center text-sm font-medium group-hover:text-blue-600">{cat.name}</p>
              </Link>
            ))}
          </div>
        </div>
